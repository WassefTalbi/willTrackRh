...
import {Notifications} from "../../core/model/notification.model";
...
export class TopbarComponent { country: any;
  selectedItem!: any;
  imageToShow: any;
  flagvalue: any;
  valueset: any;
  ...
  ngOnInit(): void {
    ...
    this.userService.getCurrentUser().subscribe(
      user => {
        ...
        this.notificationService.getNotificationsByReceiveuserId(this.currentUser?.id).subscribe(response => {
          this.notificationList = response;
          if (this.notificationList) {
            this.readNotifications = this.notificationList.filter((notification: any) => {
              console.log(notification.read);
              return notification.read === true;
            });

            this.unreadNotifications = this.notificationList.filter((notification: any) => {
              return notification.read === false;
            });

            this.newNotify = this.unreadNotifications.length;

            this.readNotifications.forEach((notification: any) => {

              this.notificationService.getUserBySenduserId(notification.senduserId).subscribe(
                (user: User) => {
                  ...
                  this.getImage();
                  this.userService.getImage(notification.user.photoProfile).subscribe(data => {
                    notification.user.photoProfileUrl = this.sanitizer.bypassSecurityTrustUrl(URL.createObjectURL(data));
                  });
                },
                error => {
                  console.error('Error fetching user for read notification', error);
                }
              );
            });

            this.unreadNotifications.forEach((notification: any) => {
              this.notificationService.getUserBySenduserId(notification.senduserId).subscribe(
                (user: User) => {
                  notification.user = user;
                  this.userService.getImage(notification.user.photoProfile).subscribe(data => {
                ...
                error => {
                  console.error('Error fetching user for unread notification', error);
                }
              );
            });
          } else {
            console.error('notificationList is undefined');
          }

          console.log(this.newNotify);

          console.log('Notifications retrieved successfully', this.notificationList);
        });
    ...
    this.wsService.onStockUpdate().subscribe(stock => {
      if (stock.quantite < stock.piece.seuilMin) {
        this.toastr.warning(`⚠️ ${stock.piece.nom} sous seuil !`, 'Alerte stock');
        this.alertes.push(stock);
      }
    });
  ...
  checkNotification(notification: Notifications): void {
    if (notification.id) {
  ...
  markAllNotificationsAsRead(): void {
    if (this.notificationList) {
      this.notificationList.forEach((notification: Notifications) => {
        if (notification.id) {
          this.notificationService.markNotificationAsRead(notification.id).subscribe(
            () => {
              notification.isRead = true;
      ...
      // Update the unread and read notifications lists
      this.unreadNotifications = this.notificationList.filter((notification: any) => !notification.isRead);
      this.readNotifications = this.notificationList.filter((notification: any) => notification.isRead);
      this.newNotify = this.unreadNotifications.length;
    } else {
      console.error('notificationList is undefined');
    }
