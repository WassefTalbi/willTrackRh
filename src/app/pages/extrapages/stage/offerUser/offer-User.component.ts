...
@Component({
  ...
  styleUrl: './offer-User.component.scss'
})
export class OfferUserComponent {
  term: any
  // bread crumb items
 ...
 // itemsPerPage = 2; // Set the number of items per page
  ...
  ngOnInit(): void {
    ...
    this.offreService.getAllOffres().subscribe(data => {
      const jobOffers = data.filter((offer: any) => offer.offerType === 'SUMMER_INTERSHIP');
...
console.log(this.editData)
  ...
  if (this.term) {
    ...
    this.pageChanged({ page: 1, itemsPerPage: 5 });
  }
  // noResultElement
