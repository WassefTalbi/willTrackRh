...
@Component({
  ...
  styleUrl: './condidat.component.scss'
})
...
ngOnInit(): void {
  ...
  loadAllUsers(){
    this.userService.getAllCondidat().subscribe(data => {
      this.users = data;
  ...
  blockUser(id: number): void {
    ...
    this.userService.blockUser(id).subscribe((response) => {
      this.toastr.success('condidat blocked avec succès!', 'Succès');
      this.loadAllUsers();
     /* const user = this.users.find((user: any) => user.id === id);
      ...
      this.toastr.error('Une erreur s\'est produite lors du blockage condidat.', 'Erreur');
  ...
  unblockUser(id: number): void {
    this.userService.unblockUser(id).subscribe((response) => {
      this.toastr.success('condidat unblocked avec succès!', 'Succès');
      this.loadAllUsers();
      /*const user = this.users.find((user: any) => user.id === id);
      ...
      this.toastr.error('Une erreur s\'est produite lors du deblockage condidat.', 'Erreur');
  ...
  onChangecondidat(event: any) {
    this.filecondidat = event.target.files[0];
  }
  addUserModalHide(){
  ...
  onSubmit() {
    ...
    const registerData = new FormData();
     ...
     /*A-Z if (this.filecondidat) {
        registerData.append('photoProfile', this.filecondidat);
      }*/
    // Call AuthService method to register user
    this.authService.registerCondidat(registerData).subscribe(
      (response) => {
        this.toastr.success('condidat ajouté avec succès!', 'Succès');
      ...
      (error) => {
        if (error.status === 400) {
          ...
          this.toastr.error('Erreur lors de l\'ajout de condidat.', 'Erreur'); 
  ...
  deleteUser() {
    this.userService.removeUser(this.deleteId).subscribe(data=>{
      this.toastr.success('condidat supprimer avec succès!', 'Succès');
      this.loadAllUsers();
      this.removeItemModal?.hide()
    },error=>{
      this.toastr.error('Erreur lors du supprision de condidat.', 'Erreur'); 
      console.log(error)
    });
  }
