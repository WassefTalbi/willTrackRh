...
ngOnInit(): void {
  ...
  loadAllUsers(){
    this.userService.getAllRh().subscribe(data => {
      this.users = data;
      console.log('the data of the users list',data)
  ...
  blockUser(id: number): void {
    ...
    this.userService.blockUser(id).subscribe((response) => {
      this.toastr.success('rh blocked avec succès!', 'Succès');
      this.loadAllUsers();
     /* const user = this.users.find((user: any) => user.id === id);
      ...
      this.toastr.error('Une erreur s\'est produite lors du blockage rh.', 'Erreur');
  ...
  unblockUser(id: number): void {
    this.userService.unblockUser(id).subscribe((response) => {
      this.toastr.success('rh unblocked avec succès!', 'Succès');
      this.loadAllUsers();
      /*const user = this.users.find((user: any) => user.id === id);
      ...
      this.toastr.error('Une erreur s\'est produite lors du deblockage rh.', 'Erreur');
  ...
  onChangerh(event: any) {
    this.filerh = event.target.files[0];
  }
  addUserModalHide(){
  ...
  onSubmit() {
    ...
    const registerData = new FormData();
      ...
      if (this.filerh) {
        registerData.append('photoProfile', this.filerh);
      }
    // Call AuthService method to register user
    this.authService.registerRh(registerData).subscribe(
      (response) => {
        this.toastr.success('rh ajouté avec succès!', 'Succès');
      ...
      (error) => {
        if (error.status === 400) {
          ...
          this.toastr.error('Erreur lors de l\'ajout de rh.', 'Erreur'); 
  ...
  deleteUser() {
    this.userService.removeUser(this.deleteId).subscribe(data=>{
      this.toastr.success('rh supprimer avec succès!', 'Succès');
      this.loadAllUsers();
      this.removeItemModal?.hide()
    },error=>{
      this.toastr.error('Erreur lors du supprision de rh.', 'Erreur'); 
      console.log(error)
    });
  }
